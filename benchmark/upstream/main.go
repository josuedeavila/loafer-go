// Command upstream runs the SQS cost benchmark against the published upstream
// loafer-go module (no batching), so its SQS API call counts can be compared
// against this repository's fork (see ../fork).
package main
...
func main() {
	...
	queueName := envOr("QUEUE_NAME", "bench-upstream")
	...
	handler := func(_ context.Context, _ loafergo.Message) error {
		if processed.Add(1) >= int64(messageCount) {
			// grace period so any in-flight visibility extension goroutines settle
	...
	printReport("upstream (unbatched)", messageCount, elapsed, countingClient.Counts())
}
